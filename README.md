## Background: Workshop Setup
- [Exercise 1 - Accessing a Kubernetes cluster with IBM Cloud Container Service](exercise-1/README.md)

## Background: Exploring Kubernetes

- [Exercise 2 - Deploying a microservice application to Kubernetes](exercise-2/README.md)
- [Exercise 3 - Inspecting a Kubernetes deployment](exercise-3/README.md)

## Istio Service Mesh on Kubernetes

- [Exercise 4 - Installing Istio](exercise-4/README.md)

## Break

## Using Istio Service Mesh

- [Exercise 5 - Deploying a microservice application with Istio Proxies](exercise-5/README.md)
- [Exercise 6 - Request routing](exercise-6/README.md)
- [Exercise 7 - Telemetry](exercise-7/README.md)
- [Exercise 8 - Fault injection](exercise-8/README.md)
- [Exercise 9 - Traffic Shifting](exercise-9/README.md)

## Credits
These workshop exercises were inspired by https://github.com/szihai/istio-workshop and the Istio bookinfo samples.

